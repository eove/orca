...
The 📢`organiser` asks all `share holders` (including 👥`team members`) to check that the [hardware token](https://eove.github.io/orca/v0.1.0/hardware_tokens.html) they own has its [public key](https://eove.github.io/orca/v0.1.0/gpg_public_key.html) recorded in the env-specific directory located under [`share_holders_keys/`] in this repository and if it needs to be updated or added, they should do it via a signed commit.
