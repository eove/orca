...
This is made easy thanks to the fact O.R.CA offers [a way to test these scripts in VMs](https://github.com/eove/orca/docs/testing). On top of this, we heavily recommend to have a preproduction PKI to validate everything before running it on the production PKI.
...
`A person inside your organisation` is anyone that has any privileged access to anything. It goes from the contractor that comes check your heaters to your C-level people.
...
If one person has full admin access to all the hardware from which the hardware will be selected, then they may deploy their attack everywhere.
...
We could by a new computer for each ceremony. The whole team go by it together and open it together. That way we are sure nobody touched it before (a supply chain attack of that scale is considered a state-level attack and is out of the scope of O.R.CA).
